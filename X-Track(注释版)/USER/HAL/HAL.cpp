...
void HAL::HAL_Init()
{
    ...
    FaultHandle_Init(); // Cortex-M芯片错误追踪库，没啥用
    Memory_DumpInfo();  // 看内存大小的，没啥用
    ...
    taskManager.Register(Power_EventMonitor, 100); // 注册任务
    taskManager.Register(GPS_Update, 200);
    taskManager.Register(SD_Update, 500);
    taskManager.Register(Memory_DumpInfo, 1000);

    Timer_SetInterrupt(CONFIG_HAL_UPDATE_TIM, 10 * 1000, HAL_TimerInterrputUpdate); // 10ms时间片里运行更新函数
